...
exports.getPageProfil = async (req, res) => {
  ...
  const nbCommentaire = await query(`SELECT Comment.content, Comment.date, User.pseudo FROM Comment LEFT JOIN User ON Comment.author_id = User.id WHERE author_id = ${ req.session.user.id }`)
  ...
  const recupArticle = await query(`SELECT Article.title, Article.description, Article.date, User.pseudo FROM Article LEFT JOIN User ON Article.author_id = User.id WHERE author_id = ${ req.session.user.id }`)
